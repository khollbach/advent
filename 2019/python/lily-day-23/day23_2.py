...
class NIC:
    def __init__(self, pid: int, msg_buffers: List[Queue], nat_buffer: Queue):
        ...
        # Track the number of times this cpu has asked for
        # input when the incoming message buffer was empty.
        self._input_count = 0
    ...
    def get(self) -> int:
        ...
        try:
            ...
            time.sleep(0.1)
            self._input_count += 1
            if self._input_count % 15 == 0:
                #print(f"\t\t{self._pid} is alive")
                pass
            #time.sleep(1 / (10**9))
            return -1
...
class NAT:           
    ...
    def _system_idle(self) -> bool:
        ...
        '''
        for q in self._msg_buffers:
            if not q.empty():
                return False
        return True
        
    def _system_resume(self) -> None:
        assert not self.incoming.empty()
...
def main():
    ...
    # Run the NAT in its own thread.
    t = Thread(target=nat.run)
    t.start()
    threads.append(t)
